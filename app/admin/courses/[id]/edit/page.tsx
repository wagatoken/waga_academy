...
export default function EditCourse() {
  ...
  useEffect(() => {
    // Use id from params, fetch course directly by id
    const id = params?.id as string | undefined
    if (!id) return
    setIsSaving(true)
    fetch(`/api/courses/${id}/full`)
      .then(async (res) => {
        const data2 = await res.json()
        console.log(data2)
        if (data2?.data) {
          setCourseId(data2.data.id)
          setFormData({
            title: data2.data.title || "",
            slug: data2.data.slug || "",
            description: data2.data.description || "",
            instructor: data2.data.instructor || "",
            difficulty_level: data2.data.difficulty_level || "",
            category: data2.data.category || "",
            duration: data2.data.duration || "",
            language: data2.data.language || "english",
            image_url: data2.data.image_url || "",
            prerequisites: data2.data.prerequisites || "",
            target_audience: data2.data.target_audience || "",
            learning_objectives: data2.data.learning_objectives || "",
            tags: data2.data.tags || "",
            status: data2.data.status || "Draft",
            visibility: data2.data.visibility || "public",
            certificate: data2.data.certificate ?? true,
            is_published: data2.data.is_published ?? false,
            additional_resources: data2.data.additional_resources || "",
          })
          // Map modules and lessons to UI format
          setModules(Array.isArray(data2.data.modules) ? data2.data.modules.map((mod: any) => ({
            id: mod.module_id,
  ...
  const handleSaveBasic = async () => {
    ...
    try {
      ...
      if (result?.data && !courseId) setCourseId(result.data.id)
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold web3-gradient-text">Edit Course</h1>
        <p className="text-muted-foreground mt-2">Update and manage your course details</p>
      </div>
      {isSaving ? (
